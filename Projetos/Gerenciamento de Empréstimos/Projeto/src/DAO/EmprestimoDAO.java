...
public class EmprestimoDAO {
    ...
    public void editar(Emprestimo emprestimo) {
        String sql = "UPDATE Emprestimos SET estado = ? WHERE id = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, emprestimo.getEstado());
    ...
    public Emprestimo verificarEstadoUltimoEmprestimo(int idItem) {
        String sql = "SELECT id, estado FROM Emprestimos WHERE id_item = ? ORDER BY id DESC LIMIT 1";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
